...
module.exports = { 
	...
	run: async (client, message, args) => {
		...
		const embed = new MessageEmbed()
			.setAuthor(`Pitch set to: ${args[0]}`, 'https://cdn.discordapp.com/emojis/758423098885275748.gif')
			.setColor('#000001');
		await delay(5000);
		msg.edit({ content: " ", embeds: [embed] });
			console.log(chalk.magenta(`[COMMAND] Pitch used by ${message.author.tag} from ${message.guild.name}`));
	}
};