...
module.exports = async (client, message) => { 
    ...
    if (!client.dev.includes(message.author.id) && client.dev.length > 0) { 
      ...
      console.log(chalk.bgRedBright(`[WARN] ${message.author.tag} trying request the command from ${message.guild.name}`)); 
      return;
    }

    console.log(chalk.magenta(`[COMMAND] ${command.config.name} executed by ${message.author.tag} | [${client.user.tag}] in ${message.guild.name} (${message.guild.id})`));
